...
public class FrmSpravMat extends JDialog
{
	private DBManager manager;
	private JTable tblSprMat;
	private JButton btnClose;
	private SpravMatTableModel tblModel;
	private TableRowSorter<SpravMatTableModel> tblSorter;
	...
	public FrmSpravMat(DBManager manager)
	{
		super();
	...
	private JToolBar getToolBar()
	{
		...
		btnEdit.setToolTipText("Изменить данные материала");
	...
	private void createGUI()
	{
		...
		tblSprMat = new JTable();
		// Создание объекта табличной модели на базе
		// сформированного списка
		tblSprMat.setModel(tblModel = new SpravMatTableModel(sprmat));
		// Создание объекта сортировки для табличной модели
		RowSorter<SpravMatTableModel> sorter = new TableRowSorter<SpravMatTableModel>(tblModel);
		// Назначение объекта сортировки таблице
		tblSprMat.setRowSorter(sorter);
		// Задаем параметры внешнего вида таблицы
		// Выделение полосой всей текущей строки
		tblSprMat.setRowSelectionAllowed(true);
		// Задаем промежутки между ячейками
		tblSprMat.setIntercellSpacing(new Dimension(0, 1));
		// Задаем цвет сетки
		tblSprMat.setGridColor(new Color(170, 170, 255).darker());
		// Минимальные и максимальные размеры колонок 0 и 1
		tblSprMat.getColumnModel().getColumn(0).setMinWidth(30);
		tblSprMat.getColumnModel().getColumn(0).setMaxWidth(50);
		// Автоматическое определение ширины последней колонки
		tblSprMat.setAutoResizeMode(JTable.AUTO_RESIZE_LAST_COLUMN);
		// Возможность выделения только 1 строки
		tblSprMat.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		// Создание области прокрутки и вставка в нее таблицы
		JScrollPane scrlPane = new JScrollPane(tblSprMat);
	...
	private void editSpravMat()
	{
		int index = tblSprMat.getSelectedRow();
		if (index == -1)
			return;
		// Преобразование индекса таблицы в индекс модели
		int modelRow = tblSprMat.convertRowIndexToModel(index);
		// Получаем объект из модели по индексу
		Sprav_mat sprm = sprmat.get(modelRow);
		BigDecimal key = sprm.getId_sprav_mat();
		// Создание объекта окна редактирования
		EdsSpravMatDialog dlg = new EdsSpravMatDialog(this, sprm, manager);
	...
	private void addSpravMat()
	{
		// Создание объекта окна редактирования
		EdsSpravMatDialog dlg = new EdsSpravMatDialog(this, null, manager);
		// Вызов окна и проверка кода возврата
		if (dlg.showDialog() == JDialogResult.OK)
		{
			// Создание нового объекта по введенным данным
			Sprav_mat sprm = dlg.getSprMat();
			// Добавление его в табличную модель
			tblModel.addRow(sprm);
		}
	}

	// Удаление текущего продукта
	private void deleteSpravMat()
	{
		...
		try
		{
			...
			BigDecimal kod = sprm.getId_sprav_mat();
			// Вызов метода менеджера для удаления строки
			if (manager.deleteSpravMat(kod))
			{
